<?php
/**
* Main class for CrZ, holds everything.
*
* @package CrZCore
*/
class CrZC implements ISingleton {
  ...
  public function FrontControllerRoute() {
    ...
    if($controllerExists && $controllerEnabled && $classExists) {
      ...
      public function ThemeEngineRender() {
        ...
        $themePath  = CRZ_INSTALL_PATH . '/' . $this->config['theme']['path'];
        ...
        if(isset($this->config['theme']['parent'])) {
          $parentPath = CRZ_INSTALL_PATH . '/' . $this->config['theme']['parent'];
          $parentUrl  = $this->request->base_url . $this->config['theme']['parent'];
        }
       
        // Add stylesheet name to the $crz->data array
        $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
       
        // Make the theme urls available as part of $crz
    ...
    if(is_array($this->config['theme']['menu_to_region'])) {
      foreach($this->config['theme']['menu_to_region'] as $key => $val) {
        ...
        $crz = &$this;
        // First the default CrZ themes/functions.php
        include(CRZ_INSTALL_PATH . '/themes/functions.php');
        ...
        // Extract $crz->data to own variables and handover to the template file
...
*/
  public function AddMessage($type, $message, $alternative=null) {
    ...
    $this->session->AddMessage($type, $message);
       ...
       /**
       * Draw HTML for a menu defined in $crz->config['menus'].
       *
