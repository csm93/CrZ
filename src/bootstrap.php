...
function autoload($aClassName) {
  $classFile = "/src/{$aClassName}/{$aClassName}.php";
   $file1 = CRZ_SITE_PATH . $classFile;
   $file2 = CRZ_INSTALL_PATH . $classFile;
   if(is_file($file1)) {
      require_once($file1);
...
function htmlent($str, $flags = ENT_COMPAT) {
  return htmlentities($str, $flags, CrZC::Instance()->config['character_encoding']);
}
...
function exception_handler($exception) {
  echo "CrZ: Uncaught exception: <p>" . $exception->getMessage() . "</p><pre>" . $exception->getTraceAsString(), "</pre>";
}
