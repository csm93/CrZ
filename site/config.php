 ...
 */
$crz->config['debug']['crz'] = false;
$crz->config['debug']['session'] = false;
$crz->config['debug']['timer'] = true;
$crz->config['debug']['db-num-queries'] = true;
$crz->config['debug']['db-queries'] = true;
$crz->config['debug']['display-crz'] = true;
 ...
 * Define session name
 */
$crz->config['session_name'] = preg_replace('/[:\.\/-_]/', '', __DIR__);
$crz->config['session_key']  = 'crz';
...
* Set database(s).
*/
$crz->config['database'][0]['dsn'] = 'sqlite:' . CRZ_SITE_PATH . '/data/.ht.sqlite';

/**
* How to hash password of new users, choose from: plain, md5salt, md5, sha1salt, sha1.
*/
$crz->config['hashing_algorithm'] = 'sha1salt';

/**
* Allow or disallow creation of new user accounts.
*/
$crz->config['create_new_users'] = true;
...
* Define menus.
*
* Create hardcoded menus and map them to a theme region through $crz->config['theme'].
*/
$crz->config['menus'] = array(
  'navbar' => array(
...
* Template files can reside in the parent or current theme, the CrzC::ThemeEngineRender()
...
*
* path: Path to current theme, relativly CRZ_INSTALL_PATH, for example themes/grid or site/themes/mytheme.
